  ...
  const uploadAll = async () => {
    ...
    try {
      ...
      if (successCount === files.length) {
        setStatus('Το gallery ενημερώθηκε με επιτυχία.');
      } else if (successCount > 0) {
        setStatus(`Ολοκληρώθηκαν ${successCount} από τα ${files.length} αρχεία.`);
      } else {
        setStatus('Αποτυχία ανεβάσματος όλων των αρχείων.');
      }
    } catch (err: any) {
    ...
    <div className="card p-6 space-y-4">
      ...
      >
        <p className="text-sm text-muted mb-3">
          Σύρετε φωτογραφίες ή βίντεο εδώ ή πατήστε για επιλογή.
        </p>
        <input
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-64 overflow-y-auto">
            {files.map(f => (
              <div
                key={f.id}
                className="relative border border-subtle rounded-lg overflow-hidden bg-[rgba(0,0,0,0.03)]"
              >
                <img
                  src={f.previewUrl}
                  alt={f.file.name}
