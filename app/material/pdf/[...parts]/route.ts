...
import {
  ...
  getMaterialPdfPrefix,
  isMaterialPdfKey,
  materialPdfContentType,
} from "@/lib/material";

async function findPdfKey(courseSlug: string, year: string, lesson: string, name: string) {
  const prefix = getMaterialPdfPrefix(courseSlug, year, lesson);
  ...
  do {
    ...
    const match = (res.Contents || []).find((item) => {
      const key = item.Key || "";
      return isMaterialPdfKey(key) && displayMaterialFilename(key) === name;
    });
...
function buildHeaders(name: string, res: GetObjectCommandOutput) {
  ...
  headers.set("Content-Type", res.ContentType || materialPdfContentType());
...
export async function GET(
  ...
  const key = await findPdfKey(courseSlug, year, lesson, name);
